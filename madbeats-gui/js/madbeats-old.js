(function() {
  ...
  function draw() {
    ...
    // TODO: calcular en función de ancho y alto?
    var squareSide = 5,
        detailWidth = 200,
        detailHeight = 110,
        triangleSize = squareSide * 4,
        tooltipMargin = 10;
    ...
    var tooltipRotateOut = function (d) {
      return d3.interpolateString("rotate(0,"+(d.x+(squareSide/2))+","+(d.y+(squareSide/2))+")",
        "rotate("+randomInRange(-720, -1440)+","+(d.x+(squareSide/2))+","+(d.y+(squareSide/2))+")");
    };
    ...
    //var ready1Min = function (error, tweets, flightsData) {
    var ready1Min = function (error, tweets) {
      // Add tweets.
    ...
    var ready5Min = function (error, mps, checkins, photos) {
      // Add traffic data.
      ...
      checkin.enter()
             ...
             /*.on("mouseover", function (d) {
               ...
               d3.select(this)
                 ...
                 .attr("y", d.y < (height/2) ? d.y : d.y - detailHeight)
                 .attr("transform", null)
                 .attr("class", "checkin info-border");
                tooltips.append("text")
                        .attr("x", d.x < (width/2) ? d.x : d.x - detailWidth)
                        .attr("y", d.y < (height/2) ? d.y : d.y - detailHeight)
                        .attr("dy", 1)
      ...
      photo.enter()
        ...
        /*.on("mouseover", function (d) {
          ...
          d3.select(this)
            ...
            .ease("bounce")
            .attr("width", detailWidth)
            .attr("height", detailHeight * 2.5)
            .attr("x", d.x < (width/2) ? d.x : d.x - detailWidth)
            .attr("y", d.y < (height/2) ? d.y : d.y - detailHeight)
            .attr("transform", null)
            .attr("class", "photo info-border");
          tooltips.append("image")
            .attr("class", "tooltipPhoto")
            .attr("xlink:href", d.photo)
            .attr("x", d.x < (width/2) ? d.x + tooltipMargin : d.x - detailWidth + tooltipMargin)
            .attr("y", d.y < (height/2) ? d.y : d.y - detailHeight)
        ...
        .on("mouseout", function (d) {
          tooltips.selectAll("text").remove();
          tooltips.selectAll("image").remove();
          d3.select(this)
            .transition()
            .duration(500)
            .ease("bounce")
